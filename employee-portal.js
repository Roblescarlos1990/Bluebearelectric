(function(){
  ...
  async function insert(table,row,success){ const {error}=await client.from(table).insert(row); if(error){console.error(error); msg(`Could not save ${table.replaceAll('_',' ')}. Check V6-7-SQL policies.`,false); return;} msg(success); await loadPortal(); }
