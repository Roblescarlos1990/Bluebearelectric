(function(){
  ...
  async function createCarousel(root){
    ...
    items.forEach((item,index)=>{
      ...
      card.type='button';card.className='bb3d-card';card.dataset.index=index;
      ...
      card.onclick=()=>{if(index===active)openLightbox(item);else go(index)};
      stage.appendChild(card);
      const thumb=document.createElement('button');
      thumb.type='button';thumb.className='bb3d-thumb';thumb.dataset.index=index;
      thumb.innerHTML=`<img src="${escapeHTML(item.src)}" alt=""><span>${index+1}</span>`;
      thumb.onclick=()=>go(index);thumbs.appendChild(thumb);
    });
    const cards=[...stage.children], thumbBtns=[...thumbs.children];
    ...
    function go(index){
      active=(index+items.length)%items.length;render();
    }
    ...
    root.addEventListener('wheel',e=>{
      if(Math.abs(e.deltaX)>Math.abs(e.deltaY)||e.shiftKey){e.preventDefault();e.deltaX+e.deltaY>0?next():prev()}
    },{passive:false});
    ...
    root.addEventListener('pointermove',e=>{
      ...
      root.style.setProperty('--light-x',`${(x+.5)*100}%`);
      root.style.setProperty('--light-y',`${(y+.5)*100}%`);
    });
    root.addEventListener('pointerleave',()=>{
      root.style.removeProperty('--scene-ry');root.style.removeProperty('--scene-rx');
    });
    function resetAuto(){
      clearInterval(timer);
    ...
    function openLightbox(item){
      const modal=document.createElement('div');modal.className='bb3d-lightbox';
