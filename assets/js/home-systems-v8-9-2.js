(function () {
  ...
  async function load() {
    const fallback = await fetch(localManifest)
      .then((r) => r.json())
      .catch(() => ({ systems: {} }));
  ...
  function bind() {
    document
      .querySelectorAll('[data-home-system]')
      .forEach((b) => b.addEventListener('click', () => show(b.dataset.homeSystem, 0)));
    document.querySelectorAll('[data-open-home-system]').forEach((card) => {
      const open = () => {
        show(card.dataset.openHomeSystem, 0);
        $('#home-zones')?.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth', block: 'start' });
      };
    ...
    stage.addEventListener('pointermove', (e) => {
      ...
      stage.style.setProperty('--viewer-ry', `${x * 3.2}deg`);
      stage.style.setProperty('--viewer-rx', `${y * -2.3}deg`);
    });
    stage.addEventListener('pointerleave', () => {
      stage.style.removeProperty('--viewer-ry');
      stage.style.removeProperty('--viewer-rx');
    });
  }
  ...
  function show(key, index = 0) {
    ...
    current = key;
    activeIndex = Math.max(0, Math.min(index, systems[key].items.length - 1));
    document
      .querySelectorAll('[data-home-system]')
      .forEach((b) => b.classList.toggle('active', b.dataset.homeSystem === key));
    ...
    $('[data-home-system-features]').innerHTML = (meta.features || [])
      .map((x) => `<li>${esc(x)}</li>`)
      .join('');
    renderDeck();
    renderThumbs();
  ...
  function move(delta) {
    const items = systems[current]?.items || [];
  ...
  function renderDeck() {
    const deck = $('[data-home-system-deck]'),
      items = systems[current]?.items || [];
    ...
    deck.querySelectorAll('[data-card-index]').forEach(
      (card) =>
        (card.onclick = () => {
          ...
          i === activeIndex ? openLightbox() : show(current, i);
        }),
    );
  }
  function renderThumbs() {
    const box = $('[data-home-thumbs]'),
      items = systems[current]?.items || [];
    box.innerHTML = items
      .map(
        (x, i) =>
    ...
    box
      ...
      .forEach((b) => (b.onclick = () => show(current, Number(b.dataset.thumb))));
  }
  function openLightbox() {
    const item = systems[current]?.items?.[activeIndex];
    if (!item) return;
