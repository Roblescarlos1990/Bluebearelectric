(function(){
  ...
  let registry=[],managed=[];
  const esc=v=>String(v??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const safe=n=>String(n||'image').toLowerCase().replace(/[^a-z0-9._-]+/g,'-');
  ...
  async function loadRegistry(){
    ...
    pageSelect.innerHTML='<option value="">All pages</option>'+pages.map(p=>`<option value="${esc(p)}">${esc(p.replace(/-/g,' ').replace(/\b\w/g,m=>m.toUpperCase()))}</option>`).join('');
  }

  async function loadManaged(){
    const {data,error}=await client.from('website_photo_slots').select('*').eq('tenant_key','blue-bear-electric');
    managed=error?[]:(data||[]);
  }
  ...
  function render(){
    ...
    const filtered=registry.filter(slot=>{
      ...
      if(!q)return true;
      return `${slot.slot_key} ${slot.page_key} ${slot.alt_text} ${slot.current_src}`.toLowerCase().includes(q);
    });
    ...
    list.innerHTML=filtered.map(slot=>{
      ...
      return `<article class="photo-slot-card" data-slot="${esc(slot.slot_key)}">
        ...
        <div class="photo-slot-actions">
          ...
          ${row?`<button class="btn dark mini-btn" type="button" data-slot-toggle>${row.is_published?'Hide replacement':'Publish replacement'}</button>
  ...
  async function refresh(){status.textContent='Loading...';await Promise.all([loadRegistry(),loadManaged()]);render()}
  ...
  list.addEventListener('click',async e=>{
    ...
    if(e.target.matches('[data-slot-upload]')){
      ...
      if(!file){window.voltflowToast?.('Choose a photo first','Select a replacement image before uploading.');return}
      ...
      try{
        ...
        const up=await client.storage.from('site-media').upload(path,file,{cacheControl:'3600',upsert:false});
        ...
        const payload={tenant_key:'blue-bear-electric',slot_key:slot.slot_key,page_key:slot.page_key,alt_text:alt,storage_path:path,public_url:url,is_published:true,updated_at:new Date().toISOString()};
        const {error}=await client.from('website_photo_slots').upsert(payload,{onConflict:'tenant_key,slot_key'});
        if(error)throw error;
        window.voltflowToast?.('Photo location updated',`${slot.slot_key} now uses the uploaded image.`);
        await refresh();
      }catch(err){window.voltflowToast?.('Upload failed',err.message)}
      finally{e.target.disabled=false;e.target.textContent='Upload & Publish'}
    }
    if(e.target.matches('[data-slot-toggle]')&&row){
      await client.from('website_photo_slots').update({is_published:!row.is_published,updated_at:new Date().toISOString()}).eq('id',row.id);
      await refresh();
    }
    if(e.target.matches('[data-slot-restore]')&&row&&confirm('Restore the original repository image for this exact location?')){
