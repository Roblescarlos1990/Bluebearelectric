(function(){
  ...
  function bars(el,rows){ const max=Math.max(1,...rows.map(r=>r.value)); el.innerHTML=rows.length?rows.map(r=>`<div class="bar-row"><b>${esc(r.label)}</b><div class="bar-track"><span style="width:${Math.max(6,(r.value/max)*100)}%"></span></div><span>${r.value}</span></div>`).join(''):'<p class="small">No data yet.</p>'; }
  async function renderReports(){
    if(!client || !$('[data-report-grid]')) return;
    const {data:{session}}=await client.auth.getSession(); if(!session) return;
    const [customers,projects,gallery,items,invoices,schedule]=await Promise.all(['customers','projects','gallery','estimate_items','invoices','schedule_events'].map(count));
    ...
    const byStatus={}; (projectRows.data||[]).forEach(p=>{const k=p.status||'Planning'; byStatus[k]=(byStatus[k]||0)+1;});
    bars($('[data-status-chart]'),Object.entries(byStatus).map(([label,value])=>({label,value})));
    const byService={}; (projectRows.data||[]).forEach(p=>{const k=p.service_type||'General'; byService[k]=(byService[k]||0)+1;});
    bars($('[data-service-chart]'),Object.entries(byService).map(([label,value])=>({label,value})));
  }
  function draftFromDOM(){
    ...
    const rows=[...document.querySelectorAll('[data-estimate-list] .line-table > div')].map(r=>r.textContent.trim()).filter(Boolean);
  ...
  document.addEventListener('click',e=>{
    if(e.target.matches('[data-generate-proposal]')){
      const global=$('[data-ai-global-output]'); if(global) global.value=draftFromDOM();
    }
  });
